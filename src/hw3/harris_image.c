...
image make_1d_gaussian(float sigma)
{
    ...
    int p =(w-1)/2;
    float ratio = pow((1/(TWOPI*pow(sigma,2)))*exp((-pow(p,2)-pow(p,2))/(2*pow(sigma,2))),0.5);
    for(int i=-p;i<=p;i++){

    set_pixel(f,1,1,i,(1/(TWOPI*pow(sigma,2)))*exp((-pow(p,2)-pow(i,2))/(2*pow(sigma,2)))/ratio);
    }
...
image structure_matrix(image im, float sigma)
{
    ...
    for(int h=0;h<im.h;h++){
        for(int w=0;w<im.w;w++){
            set_pixel(S,0,h,w,pow(get_pixel(Ix,0,h,w),2));
            set_pixel(S,1,h,w,pow(get_pixel(Iy,0,h,w),2));
            set_pixel(S,2,h,w,get_pixel(Iy,0,h,w)*get_pixel(Ix,0,h,w));
...
image cornerness_response(image S)
{
    ...
    for(int h=0;h<S.h;h++){
        for(int w=0;w<S.w;w++){
            set_pixel(R,0,h,w,get_pixel(S,0,h,w)*get_pixel(S,1,h,w)-get_pixel(S,2,h,w)*get_pixel(S,2,h,w)-0.06*pow((get_pixel(S,0,h,w)+get_pixel(S,1,h,w)),2));
...
image nms_image(image im, int w)
{
    ...
    for(int h=0;h<im.h;h++){
        for(int y=0;y<im.w;y++){
                        set_pixel(r,0,h,y,get_pixel(im,0,h,y));
            for(int i=-w;i<=w;i++){
                for(int j=-w;j<=w;j++){
                    if(get_pixel(im,0,h,y)<get_pixel(im,0,h+i,y+j)){
                        set_pixel(r,0,h,y,-999999);
                    }
...
descriptor *harris_corner_detector(image im, float sigma, float thresh, int nms, int *n)
{
    ...
    *n = count; // <- set *n equal to number of corners in image.
    ...
    for (int index=0 ; index<im.h*im.w;index++){
            if(Rnms.data[index]>thresh){
                d[itr++] = describe_index(im,index);
