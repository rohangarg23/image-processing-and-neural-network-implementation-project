...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        float bestfit = 999999;
        int bestmatch;
        // TODO: for every descriptor in a, find best match in b.
        for(i=0;i<bn;i++){
            if(l1_distance(a[j].data,b[i].data,a[j].n)<bestfit){
                bestfit=l1_distance(a[j].data,b[i].data,a[j].n);
                bestmatch = i;

        }
        }
        // record ai as the index in *a and bi as the index in *b.
        int bind = bestmatch; // <- find the best match
        m[j].ai = j;
        m[j].bi = bind; // <- should be index in b.
        m[j].p = a[j].p;
        m[j].q = b[bind].p;
        m[j].distance = bestfit; // <- should be the smallest L1 distance!
    }
    ...
    int endpointer = an-1;
    for(int i=0;i<an;i++){
        if(seen[m[count].bi]==1){
            for(int j=count;j<endpointer;j++){
                m[j]=m[j+1];
            }
            endpointer--;
            
        }
        if(seen[m[count].bi]==0){
            seen[m[count].bi]=1;
            count++;
        }
    }
...
point project_point(matrix H, point p)
{
    ...
    multiplied_matrix = matrix_mult_matrix(H,c);
    // Remember that homogeneous coordinates are equivalent up to scalar.
    // Have to divide by.... something...
    point q = make_point(0, 0);
    q.x=multiplied_matrix.data[0][0]/multiplied_matrix.data[2][0];
    q.y=multiplied_matrix.data[1][0]/multiplied_matrix.data[2][0];
    return q;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    if(point_distance(m[i].q,project_point(H,m[i].p))<thresh){
        match temp = m[i];
        m[i] = m[count];
        m[count] =temp;
        count++;
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    matrix temp = make_translation_homography(256, 0);
    ...
    for(int i=0;i<k;i++){
        ...
        temp = compute_homography(m,8);
        int inliners = model_inliers(temp,m,n,thresh);
        if(inliners>best_inliners){
            Hb = compute_homography(m,inliners);
            best_inliners = inliners;

        }
        if(best_inliners>cutoff){
            break;
        }
    }
...
image combine_images(image a, image b, matrix H)
{
    ...
    //     for(j = 0; j < b.h; ++j){
    //         for(i = 0; i < b.w; ++i){
    //             set_pixel(c,k,project_point(Hinv,make_point(i,j)).y-dy,project_point(Hinv,make_point(i,j)).x-dx,get_pixel(b,k,j,i));
    //         }
    //     }
    // }
    ...
    for(k = 0; k < b.c; ++k){
        for(j = topleft.y-dy; j < botright.y-dy; ++j){
            for(i = topleft.x-dx; i < botright.x-dx; ++i){
                if(project_point(H,make_point(i+dx,j+dy)).x<b.w&&project_point(H,make_point(i+dx,j+dy)).y<b.h&&project_point(H,make_point(i+dx,j+dy)).x>=0&&project_point(H,make_point(i+dx,j+dy)).y>=0){
                    float pixel = bilinear_interpolate(b,k,project_point(H,make_point(i+dx,j+dy)).y,project_point(H,make_point(i+dx,j+dy)).x);
                    set_pixel(c,k,j,i,pixel);
                }
            }
        }
    }
