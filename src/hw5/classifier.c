...
matrix backward_layer(layer *l, matrix delta)
{
    ...
    matrix dw = matrix_mult_matrix(transpose_input,delta);
    ...
    matrix transpose_output = transpose_matrix(l->w);
    matrix dx = matrix_mult_matrix(delta,transpose_output);
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    matrix calculation_1 = axpy_matrix(momentum,l->v,l->dw);
    matrix calculation_2 = axpy_matrix(-decay,l->w,calculation_1);
    free_matrix(l->v);
    free_matrix(calculation_1);
    l->v = calculation_2;


    // Update l->w
    matrix prev_weight = l->w;
    l->w = axpy_matrix(rate,l->v,prev_weight);
    free_matrix(prev_weight);
